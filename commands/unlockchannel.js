...
module.exports = {
    ...
    run: async(client, message, args) => {
      ...
      const role = message.guild.roles.cache.find(r => r.name === 'The Traveller');
      let lockChannel = message.mentions.channels.first() || message.guild.channels.cache.get(args[0]);
      if (!lockChannel) lockChannel = message.channel;
      if (!role) {
        message.channel.send('**Please make a role!**')
      }
      ...
      message.delete();
      logchannel.send(`**The channel has been unlocked** :unlock:\n**By Moderator:** ${message.author.tag}`);
    }
}